...
import redis
import sys
import third_report_record_store

reload(sys)
sys.setdefaultencoding('utf-8')


class ThirdReportRecordClient(object):
    def __init__(self):
        # 连接redis
    ...
    def run(self):
        while 1:
            ...
            itemlen=self.r.llen("housead_RQ:housead_report_record")
            print itemlen
            #防止字符串中出现null字符无法识别
            null='unknow'
            ...
            for i in range(third_party_setting.REQUEST_BATCHNUM):
                stream = self.r.lpop("housead_RQ:housead_report_record")
                print stream
                ...
                if itemlen==0:
                    break
            #print stream'
            third_report_record_store.ThirdReportRecordStore().run(streamlist)

if __name__ == '__main__':
        third_report_record_client=ThirdReportRecordClient()
        third_report_record_client.run()
