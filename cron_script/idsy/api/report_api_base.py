...
class ReportApiBase(Base):
    ...
    def formatApiData(self, listData):
        ...
        for item in listData.values():
            ...
            if 'third_clicks' not in item:
                item["third_clicks"] = 0
            if 'is_mobgi' not in item:
                item['is_mobgi'] = 0
            item['division_rate']= self.getAppKeyRate(item['app_key'])#获取比例分成
            values = (
                item["ads_id"], item["app_key"], item["ad_type"], item["platform"], item["days"], item["hours"], item["pos_key"], item["ad_income"],
                item["third_views"], item["third_clicks"],item['division_rate'],item['is_mobgi'],item["ad_income"], item["third_views"], item["third_clicks"])
            result.append(values)
        return tuple(result)

    # appkey分成比例获取
    def getAppKeyRate(self, appKey):
        # 先判断是否是用户自定义的分成比例
        customSql = "select * from division_custom_config where app_key ='%s' and effect_date <= '%s' order by create_time desc limit 1" % (
        appKey, time.strftime('%Y-%m-%d', time.localtime(self.startTime)))
        division = self.dbConf.fetchone(customSql)
        # 如果自定义没有，那么就读取全局比例
        if division is None:
            globalSql = "select * from division_global_config where effect_date <= '%s' order by create_time desc limit 1" % ( \
                time.strftime('%Y-%m-%d', time.localtime(self.startTime)))
            division = self.dbConf.fetchone(globalSql)
            # 如果是自投数据
        if division is None:
            return 1
        if self.adsId == 'Mobgi':
            return division['own_division_rate']
        else:
            return division['ads_division_rate']
    ...
    def compareData(self,data):
        ...
        if old_data['ad_income'] is not None:
            ...
            if diff_range_income or diff_range_clicks or diff_range_views:
                ...
                return self.saveThirdApiData(data)
            else:
                return True
        else:
            #如果是None，那么直接将新数据写入
    ...
    # matt<新增第三方数据接口留存数据>
    def saveThirdApiData(self, data):
        sql = """insert into report_third_data(ads_id,app_key,ad_type,platform,days,hours,pos_key,ad_income,third_views,third_clicks,division_rate,is_mobgi)values
        (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)on duplicate key update ad_income=%s,third_views=%s,third_clicks=%s"""
    ...
    # matt<是否忽略错误，重跑时用>
    def missError(self, ads):
        date = time.strftime("%Y-%m-%d")
        sql = """select is_miss from intergration_report_third_log where date= '%s' and ads_id='%s'""" % (date, ads)
        result = self.dbConf.fetchone(sql)
        return result['is_miss']
