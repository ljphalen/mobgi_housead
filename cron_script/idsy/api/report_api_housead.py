...
class ReportApiHousead(ReportApiBase):
    ...
    def getRemoteData(self):
        ...
        for val in returnResult:
            if self.thirdAppMap.has_key(val['app_key']):
                appKey = self.thirdAppMap[val['app_key']]
                platform = self.getPlatformByAppkey(appKey)
                hour = int(val['hour'][0:2])
                adtype = configApi.MobgiType[val['ad_type']]
                key = appKey + val['date'] + ":" + str(hour) + "_" + str(adtype)
                appResult[key] = {
                    "ads_id": self.adsId,
                    "app_key": appKey,
                    "ad_type": adtype,
                    ...
                    "days": val['date'],
                    "hours": hour,
                    "ad_income": 0,#round(val['revenue']/configApi.EXCHANGE_RATE, 2),
    ...
    def getJosnData(self, requestDate):
        url = configApi.MobgiHost + '?' + self.getUrlParams(self.requestDate)
