...
class AdStatMonth(AdBase):
    ...
    def getChangeDays(self, updateTime):
        try:
            sql = """SELECT app_key,MONTH(days) as month,YEAR(days) as year,is_custom FROM report_finance where days>='2017-09-01' and update_time>=
            "%s" group by app_key,year,month,is_custom ORDER BY days;""" % (str(updateTime))
    ...
    def getStatData(self, appKey, month, year,is_custom):
        try:
            startDay, endDay = self.getMonthFirstDayAndLastDay(year, month)
            baseKpifields = ['app_key', 'MONTH(days) as month', 'YEAR(days) as years','is_custom','sum(ad_income)*6.5 as ad_income','platform']
            baseSql = "SELECT %s FROM report_finance WHERE app_key ='%s' and days >='%s' and days <='%s' and is_custom = %s;" % (
                ",".join(baseKpifields), appKey, startDay, endDay,is_custom)
            baseInfo = self.dbData.fetchone(baseSql)
            dauKpifields = ['sum(user_dau) as mau','sum(game_dau) as game_dau','is_custom']
            if self.getAppPlatform(appKey) == 1:#根据平台来区分活跃
                dauSql = "SELECT %s FROM report_dau WHERE app_key ='%s' and ad_type = 0 and days >='%s' and days <='%s' and is_custom = %s;" % (
                ",".join(dauKpifields), appKey,startDay,endDay,is_custom)
            else:#IOS没有定制渠道数据
    ...
    def run(self):
        try:
            ...
            if count == 0:
                ...
                for item in list:
                    statData, count = self.getStatData(item["app_key"], item["month"], item['year'],item['is_custom'])
                    if count > 0:
