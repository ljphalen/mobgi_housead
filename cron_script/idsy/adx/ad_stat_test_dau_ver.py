...
##定时统计新增用户
class AdTestDauVer(AdBase):
    apps = None

    def init(self):
        self.dims = ['app_key', 'platform', 'is_custom', 'app_version', 'sdk_version', 'days','conf_id','flow_id']
        self.kpis = ['new_user', 'total_user', 'user_dau', 'total_init']
        self.newUserTable = configAdx.TABLE_MID_TEST
        self.dauTable = 'report_test_dau_ver'
    ...
    def getNewUserCount(self, startDate):
        try:
            ...
            # 分app版本新增
            sql = """select app_key,is_custom,app_version,0 as sdk_version,count(distinct(user_id)) AS count,conf_id,flow_id from %s where create_date='%s' and
            action_date='%s' group by app_key,is_custom,app_version,conf_id,flow_id""" % (self.newUserTable, startDate, startDate)
            list_app, count_app = self.ibUser.fetchall(sql)
            self.info('count_app:' + str(count_app))
            # 分sdk版本新增
            sql = """select app_key,is_custom,0 as app_version,sdk_version,count(distinct(user_id)) AS count,conf_id,flow_id from %s where create_date='%s' and
            action_date='%s' group by app_key,is_custom,sdk_version,conf_id,flow_id""" % (self.newUserTable, startDate, startDate)
            list_sdk, count_sdk = self.ibUser.fetchall(sql)
            self.info('count_sdk:' + str(count_sdk))
            # 分app,sdk版本新增
            sql = """select app_key,is_custom,app_version,sdk_version,count(distinct(user_id)) AS count,conf_id,flow_id from %s where create_date='%s' and
            action_date='%s' group by app_key,is_custom,app_version,sdk_version,conf_id,flow_id""" % (self.newUserTable, startDate, startDate)
            list_app_sdk, count_app_sdk = self.ibUser.fetchall(sql)
            self.info('count_app_sdk:' + str(count_sdk))
    ...
    def getUserDauCount(self, startDate):
        try:
            ...
            eventType = 2 # 请求配置成功
            # 分app版本新增
            sql = """select app_key,is_custom,app_version,0 as sdk_version,count(distinct(user_id)) AS count,conf_id,flow_id from %s where event_type = %s and
            action_date='%s' group by app_key,is_custom,app_version,conf_id,flow_id""" % (self.newUserTable, eventType, startDate)
            list_app, count_app = self.ibUser.fetchall(sql)
            self.info('count_app:' + str(count_app))

            # 分sdk版本新增
            sql = """select app_key,is_custom,0 as app_version,sdk_version,count(distinct(user_id)) AS count,conf_id,flow_id from %s where event_type = %s and
            action_date='%s' group by app_key,is_custom,sdk_version,conf_id,flow_id""" % (self.newUserTable, eventType, startDate)
            list_sdk, count_sdk = self.ibUser.fetchall(sql)
            self.info('count_sdk:' + str(count_sdk))

            # 分app,sdk版本新增
            sql = """select app_key,is_custom,app_version,sdk_version,count(distinct(user_id)) AS count,conf_id,flow_id from %s where event_type = %s and
            action_date='%s' group by app_key,is_custom,app_version,sdk_version,conf_id,flow_id""" % (self.newUserTable, eventType, startDate)
            list_app_sdk, count_app_sdk = self.ibUser.fetchall(sql)
            self.info('count_app_sdk:' + str(count_sdk))
    ...
    def getTotalInitCount(self, startDate):
        try:
            ...
            eventType = configAdx.EVENT_INIT  # 15事件
            # 分app版本新增
            sql = """select app_key,is_custom,app_version,0 as sdk_version,count(*) AS count,conf_id,flow_id from %s where event_type = %s and
            action_date='%s' group by app_key,is_custom,app_version,conf_id,flow_id""" % (self.newUserTable, eventType, startDate)
            list_app, count_app = self.ibUser.fetchall(sql)
            self.info('count_app:' + str(count_app))

            # 分sdk版本新增
            sql = """select app_key,is_custom,0 as app_version,sdk_version,count(*) AS count,conf_id,flow_id from %s where event_type = %s and
            action_date='%s' group by app_key,is_custom,sdk_version,conf_id,flow_id""" % (self.newUserTable, eventType, startDate)
            list_sdk, count_sdk = self.ibUser.fetchall(sql)
            self.info('count_sdk:' + str(count_sdk))

            # 分app,sdk版本新增
            sql = """select app_key,is_custom,app_version,sdk_version,count(*) AS count,conf_id,flow_id from %s where event_type = %s and
            action_date='%s' group by app_key,is_custom,app_version,sdk_version,conf_id,flow_id""" % (self.newUserTable, eventType, startDate)
            list_app_sdk, count_app_sdk = self.ibUser.fetchall(sql)
            self.info('count_app_sdk:' + str(count_sdk))
            return list_app + list_sdk + list_app_sdk
    ...
    def paramData(self, data, days):
        ...
        for type in data:
            for item in data[type]:
                ...
                confId = int(item['conf_id'])
                flowId = int(item['flow_id'])
                key = app_key + str(is_custom) + str(app_version) + str(sdk_version)+str(flowId)+str(confId)
...
if '__main__' == __name__:
    ...
    while (1):
        obj = AdTestDauVer("ad_stat_test_dau_ver")
        if obj.errorFlag:
