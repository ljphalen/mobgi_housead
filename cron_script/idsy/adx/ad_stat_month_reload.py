...
class AdStatMonth(AdBase):
    ...
    def getChangeDays(self, updateTime):
        try:
            sql = """SELECT DISTINCT app_key,MONTH(days) as month,YEAR(days) as year FROM report_third_data where days>='2017-01-01' and update_time>=
    ...
    def getStatData(self,appKey,month,year,is_custom,tableName):
        try:
            ...
            baseSql = "SELECT %s FROM %s WHERE app_key ='%s' and is_custom = %s and days >='%s' and days <='%s';" % (
                ",".join(baseKpifields),tableName, appKey,is_custom,startDay, endDay)
            ...
            if self.getAppPlatform(appKey) == 1:#根据平台来区分活跃
                dauSql = "SELECT %s FROM report_dau WHERE app_key ='%s' and is_custom = %s and ad_type = 0 and days >='%s' and days <='%s';" % (
                ",".join(dauKpifields), appKey,is_custom,startDay,endDay)
            else:#IOS没有定制渠道数据
            ...
            if dauInfo['mau'] != 0 and baseInfo['ad_income'] !=0 and dauInfo['mau'] is not None and baseInfo['ad_income'] is not None:
                baseInfo['mau'] = int(dauInfo['mau'])
                baseInfo['arpu'] = decimal.Decimal(round(baseInfo['ad_income']/dauInfo['mau'],3))
            else:
                baseInfo['mau'] = 0
                baseInfo['arpu'] = 0
            if dauInfo['game_dau'] != 0 and dauInfo['game_dau'] is not None and dauInfo['mau'] !=0 and dauInfo['mau'] is not None:
    ...
    def mixData(self,customData,noCustomData):
        data = {}
        data['app_key'] = noCustomData['app_key']
        data['month'] = noCustomData['month']
        data['years'] = noCustomData['years']
        data['app_type'] = noCustomData['app_type']
        data['arpu'] = noCustomData['arpu']
        data['mau'] = noCustomData['mau']
        data['ad_income'] = noCustomData['ad_income']
        data['game_cover'] = noCustomData['game_cover']
        if customData['arpu'] is not None and self.getAppPlatform(data['app_key'] == 1):
            data['arpu'] = noCustomData['arpu'] + customData['arpu']
        if customData['mau'] is not None and self.getAppPlatform(data['app_key'] == 1):
            data['mau'] = noCustomData['mau'] + customData['mau']
        if customData['ad_income'] is not None:
            data['ad_income'] = noCustomData['ad_income'] + customData['ad_income']
        if customData['game_cover'] is not None and self.getAppPlatform(data['app_key'] == 1):
            data['game_cover'] = noCustomData['game_cover'] + customData['game_cover']
        return data
    ...
    def run(self):
        try:
            ...
            if count == 0:
                ...
                for item in list:
                    customData, count = self.getStatData(item["app_key"], item["month"], item['year'],1,'report_finance')
                    noCustomData,count = self.getStatData(item["app_key"], item["month"], item['year'],0,'report_third_data')
                    statData = self.mixData(customData,noCustomData)
                    if count > 0:
