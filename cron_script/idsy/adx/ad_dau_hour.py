...
##定时统计新增用户,活跃,触达,初始化,退出事件的小时数据
class AdxDauHour(AdBase):
    apps = None

    def init(self):
        self.dims = ['app_key', 'platform', 'is_custom', 'ad_type', 'channel_gid', 'days', 'hours']
        self.kpis = ['new_user', 'total_user', 'user_dau', 'total_init', 'exit_dau']
        self.newUserTable = configAdx.TABLE_MID_USERS
        self.dauTable = 'report_dau_hour'
    ...
    def getNewUserCount(self, startDate):
        try:
            ...
            # 不分渠道分应用
            sql = """select app_key,0 as gid,0 as ad_type,is_custom,count(distinct user_id) AS count,action_hour from %s where create_date='%s' and
            action_date='%s'  group by app_key,is_custom,action_hour""" % (self.newUserTable, startDate, startDate)
            list_app, count_app = self.ibUser.fetchall(sql)
            self.info('count_newuser_app:' + str(count_app))
            # 分渠道
            sql = """select app_key,gid,0 as ad_type,is_custom,count(distinct user_id) AS count,action_hour from %s where gid>0 and
            create_date='%s' and
            action_date='%s' group by app_key,gid,is_custom,action_hour""" % (self.newUserTable, startDate, startDate)
    ...
    def getUserDauCount(self, startDate):
        try:
            ...
            # 分appKey
            sql = """select app_key,0 as gid,is_custom,0 as ad_type,count(distinct(user_id)) AS count,action_hour from %s where event_type = %s and
            action_date='%s' group by app_key,is_custom,action_hour""" % (self.newUserTable, eventType, startDate)
            list_app, count_app = self.ibUser.fetchall(sql)
            self.info('count_dau_app:' + str(count_app))

            # 分gid
            sql = """select app_key,gid,is_custom,0 as ad_type,count(distinct(user_id)) AS count,action_hour from %s where event_type = %s and
            action_date='%s'
            and gid>0 group by app_key,gid,is_custom,action_hour""" % (self.newUserTable, eventType, startDate)
            list_gid, count_gid = self.ibUser.fetchall(sql)
            self.info('count_dau_gid:' + str(count_gid))

            # 分ad_type
            sql = """select app_key,0 as gid,is_custom,ad_type,count(distinct(user_id)) AS count,action_hour from %s where event_type = %s and
            action_date='%s'
            group by app_key,ad_type,is_custom,action_hour""" % (self.newUserTable, eventType, startDate)
            list_ad_type, count_ad_type = self.ibUser.fetchall(sql)
            self.info('count_dau_ad_type:' + str(count_ad_type))

            # 分ad_type,gid
            sql = """select app_key,gid,is_custom,ad_type,count(distinct(user_id)) AS count,action_hour from %s where event_type = %s and
            action_date='%s' and
            gid>0 group by app_key,gid,ad_type,is_custom,action_hour""" % (self.newUserTable, eventType, startDate)
    ...
    def getTotalUserCount(self, startDate):
        try:
            ...
            # 分gid
            sql = """select app_key,gid,is_custom,0 as ad_type,count(distinct(user_id)) AS count,action_hour from %s where event_type = %s and
             action_date='%s' and gid>0 group by app_key,gid,is_custom,action_hour""" % (self.newUserTable, eventType, startDate)
            list_gid, count_gid = self.ibUser.fetchall(sql)
            self.info('count_totaluser_gid:' + str(count_gid))

            # 分ad_type
            sql = """select app_key,0 as gid,is_custom,ad_type,count(distinct(user_id)) AS count,action_hour from %s where event_type = %s and
            action_date='%s' group by app_key,ad_type,is_custom,action_hour""" % (self.newUserTable, eventType, startDate)
            list_ad_type, count_ad_type = self.ibUser.fetchall(sql)
            self.info('count_totaluser_ad_type:' + str(count_ad_type))

            # 分ad_type,gid
            sql = """select app_key,gid,is_custom,ad_type,count(distinct(user_id)) AS count,action_hour from %s where event_type = %s and
            action_date='%s' and gid>0 group by app_key,gid,ad_type,is_custom,action_hour""" % (self.newUserTable, eventType, startDate)
            list_ad_type_gid, count_ad_type_gid = self.ibUser.fetchall(sql)
    ...
    def getTotalInitCount(self, startDate):
        try:
            ...
            eventType = configAdx.EVENT_INIT  # 15事件
            # # 分app版本新增
            # sql = """select app_key,0 as gid,is_custom,0 as ad_type,count(*) AS count,action_hour from %s where event_type = %s and action_date='%s'
            # group by app_key,is_custom,action_hour""" % (self.newUserTable, eventType, startDate)
            # list_app, count_app = self.ibUser.fetchall(sql)
            # self.info('count_totalinit_app:' + str(count_app))

            # 分gid
            sql = """select app_key,gid,is_custom,0 as ad_type,count(*) AS count,action_hour from %s where event_type = %s and action_date='%s'
            group by app_key,is_custom,gid,action_hour""" % (self.newUserTable, eventType, startDate)
            list_gid, count_gid = self.ibUser.fetchall(sql)
            self.info('count_totalinit_gid:' + str(count_gid))

            # 分ad_type
            sql = """select app_key,0 as gid,is_custom,ad_type,count(*) AS count,action_hour from %s where event_type = %s and action_date='%s'
            group by app_key,is_custom,ad_type,action_hour""" % (self.newUserTable, eventType, startDate)
            list_ad_type, count_ad_type = self.ibUser.fetchall(sql)
            self.info('count_totalinit_ad_type:' + str(count_ad_type))

            # 分ad_type,gid
            sql = """select app_key,gid,is_custom,ad_type,count(*) AS count,action_hour from %s where event_type = %s and action_date='%s'
            group by app_key,is_custom,ad_type,gid,action_hour""" % (self.newUserTable, eventType, startDate)
            list_ad_type_gid, count_ad_type_gid = self.ibUser.fetchall(sql)
    ...
    def getTotalExitCount(self, startDate):
        try:
            ...
            eventType = configAdx.EVENT_EXIT  # 16 exit事件

            # 分app版本
            sql = """select app_key,0 as gid,is_custom,0 as ad_type,count(*) AS count,action_hour from %s where event_type = %s and
            action_date='%s' group by app_key,is_custom,action_hour""" % (self.newUserTable, eventType, startDate)
            list_app, count_app = self.ibUser.fetchall(sql)
    ...
    def paramData(self, data, days):
        ...
        for type in data:
            for item in data[type]:
                ...
                gid = str(item['gid'])
                hour = item['action_hour']
                count = int(item['count'])
                platform = self.getAppPlatform(app_key)
                key = app_key + is_custom + gid + ad_type + str(hour)
