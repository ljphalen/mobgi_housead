...
class AdMidHousead(AdBase):
    ...
    def getRecordList(self, startPosition, nextPosition):
        ...
        fileds = " as id_range,orig_id,app_key,pos_key,ad_type,ad_sub_type,platform,charge_type,currency,event_type," \
                 "count(*) as event_count,sum(event_value) as event_value,sum(price) as amount,FROM_UNIXTIME(server_time,'%Y-%m-%d %H') as " \
                 "server_time"
        fileds = str(startPosition) + fileds
        groupby = "orig_id,app_key,pos_key,ad_type,ad_sub_type,platform,charge_type,currency,event_type,FROM_UNIXTIME(server_time,'%Y-%m-%d %H')"
...
if __name__ == '__main__':
    ...
    lastPosition = 0
    while 1:
        obj = AdMidHousead('ad_mid_housead')
        if obj.errorFlag:
            obj = None
