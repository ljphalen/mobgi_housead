...
class AdMidHour(AdBase):
    ...
    def getRecordList(self, startPosition, nextPosition):
        ...
        fileds = " as id_range,ssp_id,ads_id,app_key,pos_key,ad_type,cid,platform,event_type,app_version,sdk_version," \
                 "count(*) as event_count,sum(used_time) as event_value,sum(event_time) as event_time,FROM_UNIXTIME(server_time,'%Y-%m-%d %H') " \
                 "as server_time"
        ...
        sql = "SELECT %s FROM %s WHERE id >=%s and id<%s and ver in (1,4) group by %s " % (
