...
##定时统计新增用户,活跃,触达,初始化
class AdxTestDau(AdBase):
    apps = None

    def init(self):
        self.dims = ['app_key', 'platform', 'is_custom', 'ad_type', 'channel_gid', 'days','conf_id','flow_id']
        self.kpis = ['new_user', 'total_user', 'user_dau', 'total_init', 'exit_dau']
        self.newUserTable = configAdx.TABLE_MID_TEST
        self.dauTable = 'report_test_dau'
    ...
    def getNewUserCount(self, startDate):
        try:
            ...
            # 不分渠道分应用
            sql = """select app_key,0 as gid,0 as ad_type,is_custom,count(distinct user_id) AS count,conf_id,flow_id from %s where create_date='%s' and
            ...
            # 分渠道
            sql = """select app_key,gid,0 as ad_type,is_custom,count(distinct user_id) AS count,conf_id,flow_id from %s where gid>0 and create_date='%s' and
    ...
    def getUserDauCount(self, startDate):
        try:
            ...
            eventType = 2 # 请求配置成功
            # 分appKey
            sql = """select app_key,0 as gid,is_custom,0 as ad_type,count(distinct(user_id)) AS count,conf_id,flow_id from %s where event_type = %s and
            ...
            # 分gid
            sql = """select app_key,gid,is_custom,0 as ad_type,count(distinct(user_id)) AS count,conf_id,flow_id from %s where event_type = %s and action_date='%s'
            ...
            # 分ad_type
            sql = """select app_key,0 as gid,is_custom,ad_type,count(distinct(user_id)) AS count,conf_id,flow_id from %s where event_type = %s and action_date='%s'
            ...
            # 分ad_type,gid
            sql = """select app_key,gid,is_custom,ad_type,count(distinct(user_id)) AS count,conf_id,flow_id from %s where event_type = %s and action_date='%s' and
    ...
    def getTotalUserCount(self, startDate):
        try:
            ...
            # 分gid
            sql = """select app_key,gid,is_custom,0 as ad_type,count(distinct(user_id)) AS count,conf_id,flow_id from %s where event_type = %s and
            ...
            # 分ad_type
            sql = """select app_key,0 as gid,is_custom,ad_type,count(distinct(user_id)) AS count,conf_id,flow_id from %s where event_type = %s and
            ...
            # 分ad_type,gid
            sql = """select app_key,gid,is_custom,ad_type,count(distinct(user_id)) AS count,conf_id,flow_id from %s where event_type = %s and
    ...
    def getTotalInitCount(self, startDate):
        try:
            ...
            eventType = configAdx.EVENT_INIT  # 15事件
            # 分app版本新增
            sql = """select app_key,0 as gid,is_custom,0 as ad_type,count(*) AS count,conf_id,flow_id from %s where event_type = %s and action_date='%s'
             and gid>0 group by app_key,is_custom""" % (self.newUserTable, eventType, startDate)
            list_app, count_app = self.ibUser.fetchall(sql)
            self.info('count_totalinit_app:' + str(count_app))

            # 分gid
            sql = """select app_key,gid,is_custom,0 as ad_type,count(*) AS count,conf_id,flow_id from %s where event_type = %s and action_date='%s'
            ...
            # 分ad_type
            sql = """select app_key,0 as gid,is_custom,ad_type,count(*) AS count,conf_id,flow_id from %s where event_type = %s and action_date='%s'
            ...
            # 分ad_type,gid
            sql = """select app_key,gid,is_custom,ad_type,count(*) AS count,conf_id,flow_id from %s where event_type = %s and action_date='%s'
    ...
    def getTotalExitCount(self, startDate):
        try:
            ...
            # 分app版本
            sql = """select app_key,0 as gid,is_custom,0 as ad_type,count(*) AS count,conf_id,flow_id from %s where event_type = %s and
            action_date='%s'  and ad_type>0 group by app_key,is_custom""" % (self.newUserTable, eventType, startDate)
            list_app, count_app = self.ibUser.fetchall(sql)
    ...
    def paramData(self, data, days):
        ...
        for type in data:
            for item in data[type]:
                ...
                key = app_key + str(is_custom) + str(gid) + str(ad_type)+str(flowId)+str(confId)
    ...
    def run(self):
        try:
            ...
            data['exit_dau'] = self.getTotalInitCount(startDate)
...
if '__main__' == __name__:
    ...
    while (1):
        obj = AdxTestDau("ad_stat_test_dau")
        if obj.errorFlag:
