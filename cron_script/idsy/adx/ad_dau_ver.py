...
##定时统计新增用户
class AdDauVer(AdBase):
    apps = None

    def init(self):
        self.dims = ['app_key', 'platform', 'is_custom', 'app_version', 'sdk_version', 'days']
        self.kpis = ['new_user', 'total_user', 'user_dau', 'total_init']
        self.newUserTable = configAdx.TABLE_MID_USERS
        self.dauTable = 'report_dau_ver'
    ...
    def getNewUserCount(self, startDate):
        try:
            ...
            # 分app版本新增
            sql = """select app_key,is_custom,app_version,0 as sdk_version,count(distinct(user_id)) AS count from %s where create_date='%s' and
            action_date='%s' group by app_key,is_custom,app_version""" % (self.newUserTable, startDate, startDate)
            list_app, count_app = self.ibUser.fetchall(sql)
            self.info('count_app:' + str(count_app))
            # 分sdk版本新增
            sql = """select app_key,is_custom,0 as app_version,sdk_version,count(distinct(user_id)) AS count from %s where create_date='%s' and
            action_date='%s' group by app_key,is_custom,sdk_version""" % (self.newUserTable, startDate, startDate)
            list_sdk, count_sdk = self.ibUser.fetchall(sql)
            self.info('count_sdk:' + str(count_sdk))
            # 分app,sdk版本新增
            sql = """select app_key,is_custom,app_version,sdk_version,count(distinct(user_id)) AS count from %s where create_date='%s' and
            action_date='%s' group by app_key,is_custom,app_version,sdk_version""" % (self.newUserTable, startDate, startDate)
    ...
    def getUserDauCount(self, startDate):
        try:
            ...
            eventType = configAdx.EVENT_INIT  # 15事件
            # 分app版本新增
            sql = """select app_key,is_custom,app_version,0 as sdk_version,count(distinct(user_id)) AS count from %s where event_type = %s and
            action_date='%s' group by app_key,is_custom,app_version""" % (self.newUserTable, eventType, startDate)
            list_app, count_app = self.ibUser.fetchall(sql)
            self.info('count_app:' + str(count_app))

            # 分sdk版本新增
            sql = """select app_key,is_custom,0 as app_version,sdk_version,count(distinct(user_id)) AS count from %s where event_type = %s and
            action_date='%s' group by app_key,is_custom,sdk_version""" % (self.newUserTable, eventType, startDate)
            list_sdk, count_sdk = self.ibUser.fetchall(sql)
            self.info('count_sdk:' + str(count_sdk))

            # 分app,sdk版本新增
            sql = """select app_key,is_custom,app_version,sdk_version,count(distinct(user_id)) AS count from %s where event_type = %s and
            action_date='%s' group by app_key,is_custom,app_version,sdk_version""" % (self.newUserTable, eventType, startDate)
            list_app_sdk, count_app_sdk = self.ibUser.fetchall(sql)
            self.info('count_app_sdk:' + str(count_sdk))
    ...
    def getTotalInitCount(self, startDate):
        try:
            ...
            eventType = configAdx.EVENT_INIT  # 15事件
            # # 分app版本新增
            # sql = """select app_key,is_custom,app_version,0 as sdk_version,count(*) AS count from %s where ad_type=0 and event_type = %s and
            # action_date='%s' group by app_key,is_custom,app_version""" % (self.newUserTable, eventType, startDate)
            # list_app, count_app = self.ibUser.fetchall(sql)
            # self.info('count_app:' + str(count_app))

            # 分sdk版本新增
            sql = """select app_key,is_custom,0 as app_version,sdk_version,count(*) AS count from %s where ad_type=0 and event_type = %s and
            action_date='%s' group by app_key,is_custom,sdk_version""" % (self.newUserTable, eventType, startDate)
            list_sdk, count_sdk = self.ibUser.fetchall(sql)
            self.info('count_sdk:' + str(count_sdk))

            # 分app,sdk版本新增
            sql = """select app_key,is_custom,app_version,sdk_version,count(*) AS count from %s where ad_type=0 and event_type = %s and
            action_date='%s' group by app_key,is_custom,app_version,sdk_version""" % (self.newUserTable, eventType, startDate)
            list_app_sdk, count_app_sdk = self.ibUser.fetchall(sql)
            self.info('count_app_sdk:' + str(count_sdk))
    ...
    def paramData(self, data, days):
        ...
        for type in data:
            for item in data[type]:
                ...
                key = app_key + is_custom + app_version + sdk_version
