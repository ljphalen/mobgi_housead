...
class Util_Secoder {
	...
	public function entry() {
		...
		$ttf = realpath(BASE_PATH . 'public/static/ttf/' . mt_rand(1, 6) . '.ttf');
		...
		$_SESSION[$this->seKey]['code'] = join('', $code); // 把验证码保存到session, 验证时注意是大写  
		$_SESSION[$this->seKey]['time'] = time(); // 验证码创建时间   
