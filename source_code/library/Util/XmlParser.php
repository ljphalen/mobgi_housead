...
class Util_XmlParser {
	...
	public function __construct($version = '1.0', $encode = 'utf-8') {
		if (!class_exists('DOMDocument')) throw new Exception('DOMDocument is not exist.');
	...
	public function parseToXml($source, $charset = 'utf8') {
		switch (gettype($source)) {
			...
			case 'array':
				...
				break;
			case 'string':
				$source = Util_Convert::convert($source, 'utf8', $charset);
			default:
	...
	protected function arrayToXml($arr, $charset, $dom = null) {
		foreach ($arr as $key => $val) {
			...
			if (is_string($val)) {
				$val = Util_Convert::convert($val, 'utf8', $charset);
