...
class Util_File {
	...
	public static function savePhpData($fileName, $data, $isBuildReturn = true, $method = self::READWRITE, $ifLock = true) {
		...
		if (!$isBuildReturn && is_array($data)) {
			foreach ($data as $key => $value) {
				...
				$temp .= "\$" . $key . " = " . Util_String::varToString($value) . ";\r\n";
			}
			$temp .= "\r\n?>";
		} else {
			($isBuildReturn) && $temp .= " return ";
			$temp .= Util_String::varToString($data) . ";\r\n?>";
		}
	 ...
	 * @return boolean
	 */
	public static function logFile($fileName, $data, $method = self::APPEND_WRITE, $ifLock = true,  $ifChmod = true) {
		if (!$handle = fopen($fileName, $method)) return false;
		$ifLock && flock($handle, LOCK_EX);
		$writeCheck = fwrite($handle, date("Y-m-d H:i:s").":".$data."\r\n");
		$ifLock && flock($handle, LOCK_UN);
		fclose($handle);
		$ifChmod && chmod($fileName, 0777);
		return $writeCheck;
	}
