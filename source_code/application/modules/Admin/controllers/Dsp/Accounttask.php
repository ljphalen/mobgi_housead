...
class Dsp_AccounttaskController extends Admin_BaseController {
	public $actions = array(
		'manageUrl'        => '/Admin/Dsp_Accounttask/manage',
		'auditUrl'         => '/Admin/Dsp_Accounttask/audit',
		'detailUrl'        => '/Admin/Dsp_Accounttask/detail',
		'singleaddPostUrl' => '/Admin/Dsp_Accounttask/singleaddPost',
		'batchaddPostUrl'  => '/Admin/Dsp_Accounttask/batchaddPost',
		'uploadUrl'        => '/Admin/Dsp_Accounttask/uploadbatchcsv',
		'uploadPostUrl'    => '/Admin/Dsp_Accounttask/uploadbatchcsvPost',
		'auditPostUrl'     => '/Admin/Dsp_Accounttask/auditPost',
		'exportdetailUrl'  => '/Admin/Dsp_Accounttask/exportdetail',
	);
	...
	public function auditPostAction() {
		...
		if ($auditstate == 'checked_success') {
			...
			$info['auditby']    = $adminInfo['user_name'];
			$info['auditmsg']   = '审核通过';
			...
			$detailinfo['auditby']         = $adminInfo['user_name'];
			...
			foreach ($list as $item) {
				...
				$successFlag        = false;
				$userInfo           = Admin_Service_UserModel::getBy(array('email' => $item['email']));
				$userId             = $userInfo['user_id'];
				$virtualaccountinfo = Advertiser_Service_AccountDetailModel::getBy(array('uid' => $userId, 'account_type' => $item['virtual_account_type']));
				...
				if ($item['opertype'] == 'recharge') {
					if (empty($virtualaccountinfo)) {
						...
						$resultvirtualdetail      = $this->saveAccountVirtualDetail($userId, $item['virtual_account_type'], $item['money'], 'normal', $item['detailid'], $item['expire_time'], $adminInfo['username']);
						if ($resultdetail && $resultvirtualdetail) {
							...
							$this->saveAccountlog($userId, $item['virtual_account_type'], $item['opertype'], $item['money'], $info['auditmsg']);
							Advertiser_Service_AccountDetailModel::delTotalbalanceCache($userId);
						} else {
						...
						$resultdetail        = Advertiser_Service_AccountDetailModel::updateAccountdetailBy(array('balance' => $balance), array('uid' => $userId, 'account_type' => $item['virtual_account_type']));
						$resultvirtualdetail = $this->saveAccountVirtualDetail($userId, $item['virtual_account_type'], $item['money'], 'normal', $item['detailid'], $item['expire_time'], $adminInfo['username']);
						if ($resultdetail && $resultvirtualdetail) {
							$success++;
							$successFlag = true;
					...
					if (empty($virtualaccountinfo)) {
						...
						$resultdetail             = Advertiser_Service_AccountDetailModel::addAccountdetail($addparam);
						Advertiser_Service_AccountTaskDetailModel::updateTaskdetail(array('taskdetailstate' => 'sended_failed'), $item['detailid']);
						$this->saveAccountlog($userId, $item['virtual_account_type'], $item['opertype'], 0, $info['auditmsg']);
						Advertiser_Service_AccountDetailModel::delTotalbalanceCache($userId);
					} else {
						$balance                    = $virtualaccountinfo['balance']-$item['money'];
				...
				/*管理端操作日志start*/
				$this->mOperateData .= '任务名：'.$taskinfo['taskname'].','.$Admin_account_opertype_config[$item['opertype']].',任务详情id：'.$item['detailid'].',交易总额：'.$item['money'].', 审核成功，';
				if ($successFlag) {
					$this->mOperateData .= '发放成功';
				} else {
					$this->mOperateData .= '发放失败';
				};
				/*管理端操作日志end*/
			...
			/*管理端操作日志start*/
			$this->mOperateData .= '任务名：'.$taskinfo['taskname'].','.'审核不通过';
			/*管理端操作日志end*/
	...
	public function singleaddPostAction() {
		...
		$taskdata['applyby']     = $this->userInfo['user_name'];
		...
		$taskdetaildata['applyby']              = $this->userInfo['user_name'];
		...
		$this->mOperateData = '操作对象任务名：'.$params['taskname'].','.$Admin_account_opertype_config[$params['opertype']].',任务详情id：'.$ret.',交易总额：'.$params['money'];
		/*管理端操作日志end*/
	...
	private function checkPostSingleadd($info) {
		...
		//判断对象帐号是否存在
		$ret = Admin_Service_UserModel::getBy(array('email' => $info['email'], 'user_type' => array('IN', array(1, 2))));
		if (!$ret) {
	...
	public function batchaddPostAction() {
		...
		foreach ($list as $item) {
			...
			if ($ret) {
				$success++;
				/*管理端操作日志start*/
				$logdata['content'] .= '任务名：'.$params['taskname'].','.$Admin_account_opertype_config[$params['opertype']].',任务详情id:'.$ret.',交易总额：'.$money;
				/*管理端操作日志end*/
			}
		}
	...
	private function checkPostBatchadd($info) {
		...
		foreach ($list as $item) {
			...
			//判断对象帐号是否存在
			$result = Admin_Service_UserModel::getBy(array('email' => $email, 'user_type' => array('IN', array(1, 2))));
			if (!$result) {
