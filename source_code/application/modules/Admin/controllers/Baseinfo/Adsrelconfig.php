 ...
 *
 */
class Baseinfo_AdsrelconfigController extends Admin_BaseController {
	
	public $actions = array(
		'listUrl' => '/Admin/Baseinfo_Adsrelconfig/index',
		'addPostUrl' => '/Admin/Baseinfo_Adsrelconfig/addPost',
		'deleteUrl' => '/Admin/Baseinfo_Adsrelconfig/delete',
		'viewUrl' => '/Admin/Baseinfo_Adsrelconfig/view',
        'updateAdsposrelStateUrl' => '/Admin/Baseinfo_Adsrelconfig/updateAdsposrelState',
	    'getAdsUrl'=> '/Admin/Baseinfo_Adsrelconfig/getAdsList'
	);
	...
	public function indexAction() {
        ...
        foreach ( $appList as $key => $value ) {
            ...
            // 广告商参数配置结果
            $adsAppRelRestult = MobgiApi_Service_AdsAppRelModel::getsBy ( $whereParams, array (
                    'ads_id' => 'ASC' 
            ) );
            // 广告位配置结果
            $adsPosRelResult = MobgiApi_Service_AdsPosRelModel::getsBy ( $whereParams, array (
                    'ads_id' => 'ASC'
            ) );
    	...
    	foreach (Common_Service_Const::$mAdSubTypeDesc as $adType=>$adTypeDesc){
    	      foreach ( $info [$adTypeDesc.'_ads_id'] as  $adsId ) {
    	        ...
    	        foreach ($jsonInfo as $block){
    				...
    				if($id){
    					MobgiApi_Service_AdsPosRelModel::updateByID($data, $id);
    				}else{
    					MobgiApi_Service_AdsPosRelModel::add($data);
    				}
    			} 
    		}
    	}
	...
	private function saveAdsAppRelData($post) {
		foreach (Common_Service_Const::$mAdSubTypeDesc as $adSubType=>$adSubTypeDesc){
			...
			foreach ( $post [$adSubTypeDesc . '_ads_id'] as $key => $val ) {
				...
				$data ['template_show_time'] = $post [$adSubTypeDesc.'_is_use_template']?$post [$adSubTypeDesc.'_template_show_time']:3;
				$data ['template_id'] = $post [$adSubTypeDesc.'_is_use_template']?$post [$adSubTypeDesc.'_template_id']:'';
				...
				if($id){
					MobgiApi_Service_AdsAppRelModel::updateByID($data, $id);
				}else{
					MobgiApi_Service_AdsAppRelModel::add($data);
				}
			}
		}
	}
    	...
    	foreach (Common_Service_Const::$mAdSubTypeDesc as $adType=>$adTypeDesc){
    		 foreach ( $info [$adTypeDesc.'_ads_id'] as  $adsId ) {
    			...
    			foreach ($jsonInfo as $block){
    				...
    				$result = MobgiApi_Service_AdsPosRelModel::getBy($params);
    				$id = $block['ads_pos_rel_id'];
    				if($result && !$id ){
    ...
    private function checkPostAdsParam($info){
    	...
    	foreach (Common_Service_Const::$mAdSubTypeDesc as $adType=>$adTypeDesc){
    		...
    		foreach ( $info [$adTypeDesc . '_ads_id'] as $key => $val ) {
    			...
    			$params ['ad_sub_type'] = $adType;
    			$params ['ads_id'] = $val;
    			$result = MobgiApi_Service_AdsAppRelModel::getBy($params);
	...
	public function deleteAction() {
		...
		//删除应用关联
		if(MobgiApi_Service_AdsAppRelModel::getBy($params)){
			$ret = MobgiApi_Service_AdsAppRelModel::deleteBy($params);
			if(!$ret){
				$this->output ( - 1, '删除失败1' );
			}
		}
		//删除广告位关联
		if(MobgiApi_Service_AdsPosRelModel::getBy($params)){
			$ret = MobgiApi_Service_AdsPosRelModel::deleteBy($params);
			if(!$ret){
				$this->output ( - 1, '删除失败' );
			}
		}
    ...
    public function updateAdsposrelStateAction(){
        ...
        $ret = MobgiApi_Service_AdsPosRelModel::getByID($id);
        if (!$ret) $this->output(-1, '应用非法');
        $data['state'] = $state;
        $ret= MobgiApi_Service_AdsPosRelModel::updateByID($data, $id);
