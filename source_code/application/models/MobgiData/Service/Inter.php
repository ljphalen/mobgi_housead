 ...
 *
 */
class MobgiData_Service_InterModel extends Common_Service_Report {
    ...
    public static function getChartConf($params) {
        ...
        $conf = [
            'api' => [
                "data" => '/Admin/Inter_Report/getData',
                "conf" => '/Admin/Inter_Report/updateInterKpi',
            ],
            'kpi' => self::getKpis(),
    ...
    private static function getKpis() {
        ...
        foreach (self::$dauKpis as $key => $val) {
            $group = $val['group'] ?: 'common';
            $groups[$group][] = $key;
        }
        return $groups;
    ...
    private static function getKeyMap() {
        $conf = [];
        $all = array_merge(self::$baseDims, self::$baseKpis, self::$dauKpis, self::$kpiGroup);
    ...
    protected static function expandKpi($kpis) {
        ...
        return array_intersect(array_merge(array_keys(self::$baseKpis), array_keys(self::$dauKpis), self::$extKpis), array_unique($kpis));
    }
    ...
    protected static function getUnitTable($params) {
        ...
        if (array_intersect($kpis, ['dau'])) {
            array_push($tables, 'InterDau');
        }
        if (array_intersect($kpis, ['request', 'redirect'])) {
            array_push($tables, 'InterReport');
        }
        return $tables;
    }
    ...
    protected static function getInterReportData($params) {
        $mydims = array_keys(self::$baseDims);
        $mykpis = array_keys(self::$baseKpis);
    ...
    protected static function getInterDauData($params) {

        if (array_intersect($params['dims'], ['hours', 'ads_id', 'url_id', 'url_type'])) {
            return [];
        }
        $mydims = array_keys(self::$baseDims);
        $mykpis = array_keys(self::$dauKpis);
        $dims = array_intersect($params['dims'], $mydims);
        $kpis = array_intersect($params['kpis'], $mykpis);
        $allFields = self::getFields();
        $fields = [];
