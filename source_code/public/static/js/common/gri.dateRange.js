 ...
 * @param {object}
    *            options 配置数组
 */
function pickerDateRange(contentId,inputId, options) {
    /**
     * 默认配置参数数据，每个参数涵义在后解释
     */
    var defaults = {
        aToday:inputId+'_aToday', // 今天
        aYesterday:inputId+'_aYesterday', // 昨天
        aRecent7Days:inputId+'_aRecent7Days', // 最近7天
        aRecent14Days:inputId+'_aRecent14Days', // 最近14天
        aRecent30Days:inputId+'_aRecent30Days', // 最近30天
        aRecent90Days:inputId+'_aRecent90Days', // 最近90天
        startDate:'', // 开始日期
        endDate:'', // 结束日期
        ...
        suffix:'', // 相应控件的后缀
        inputTrigger:inputId+'_input_trigger',
        ...
        replaceBtn:inputId+'_btn_compare'
    };
    // 将对象赋给__method变量
    var __method = this;
    ...
    if (this.mOpts.isSingleDay == false) {
        var data_range = {
            ta : [
                  '<div class="ta_date" id="div_'+this.inputId+'">',
                  '<span class="date_title" id="'+this.inputId+'"></span>',
                  '<a class="opt_sel" id="'+this.inputId+'_input_trigger" href="#"><i class="i_orderd"></i></a>',
                  '</div>',
                  '<span class="ta_select_day">',
                  '<a id="'+this.inputId+'_aToday" href="javascript:void(0);">今天</a>',
                  '<a id="'+this.inputId+'_aYesterday" href="javascript:void(0);">昨天</a>',
                  '<a id="'+this.inputId+'_aRecent7Days" href="javascript:void(0);">7天</a>',
                  '<a id="'+this.inputId+'_aRecent14Days" href="javascript:void(0);">14天</a>',
                  '<a id="'+this.inputId+'_aRecent30Days" href="javascript:void(0);">30天</a>',
                  '</span>',
                  '<button class="ta_btn ta_btn_primary mui-global-ml10" id="'+this.inputId+'_btn_compare">按时间对比</button>',
            ]
        }
    }else{
        var data_range = {
    ...
    $("#"+contentId).html(data_range.ta.join(""));
    ...
    /**
     * 设置回调句柄，点击成功后，返回一个时间对象，包含开始结束时间 和对比开始结束时间
     */
    ...
    $('#' + this.compareCheckboxId).bind('click', function () {
        ...
        if ($('#' + __method.compareCheckboxId).attr('checked')) {
            ...
            if ('' == scDate || '' == ecDate) {
                ...
                // 这里要和STATS_START_TIME的时间进行对比，如果默认填充的对比时间在这个时间之前 added by
                // johnnyzheng
...
//        });
    ...
    if (this.mOpts.replaceBtn && $('#' + this.mOpts.replaceBtn).length > 0) {
        ...
        if ($('#' + __method.startCompareDateId).val() != '') {
            $('#' + this.mOpts.replaceBtn).click().click();
        }
    }
 ...
 * @param {Num}
    *            period 快捷选择的时间段，今天、昨天、最近7天、最近30天
 */
pickerDateRange.prototype.getSpecialPeriod = function (period) {
    ...
    /**
     * add by OT     debug 昨天会选择到今天的bug
     */
    if(period == 1)
    	return {today:otherdayStr, otherday:otherdayStr};
    else
 ...
 * @param {Boolean}
    *            isCompare 是否是对比日期面板
 * @param {String}
    *            specialClass 特殊的样式，这里默认是常规和对比日期两种样式的重合样式
 */
...
pickerDateRange.prototype.show = function (isCompare, __method) {
    ...
    // $("#" + this.calendarId).css('top', pos.top + (offsetHeight ?
    // offsetHeight- 1 : (__method.mOpts.theme=='ta'?35:22)) + 'px');
...
pickerDateRange.prototype.close = function (btnSubmit) {
    ...
    // this.mOpts.defaultText + $('#' + this.endDateId).val()));
    if (btnSubmit) {
        // 如果是单日快捷选择
        ...
        // //在js侧就做好日期校准，以前面的日期选择的跨度为准，如果后面的跨度超过了当前可用时间，则以当前可用时间向前推 added by
        // johnnyzheng 11-29
        if (this.mOpts.theme != 'ta') {
            if ('' != $('#' + this.startCompareDateId).val() && '' != $('#' + this.endCompareDateId).val()) {
                ...
                if (_ecDateTime > nDateTime) {
                    // 如果计算得到的时间超过了当前可用时间，那么就和服务器端保持一致，将当前可用的天数向前推日期选择器的跨度 added by
                    // johnnyzheng 11-29
        ...
        // $('#' + this.inputCompareId).val($('#' +
        // this.startCompareDateId).val() + this.mOpts.defaultText + $('#' +
        // this.endCompareDateId).val());
...
pickerDateRange.prototype.fillDate = function (year, month, index) {
    ...
    if (isTaTheme) {
        ...
        // $(td).append('<span style="font-size:16px">' + year + '年' + (month + 1) +
        // '月' + '</span>');
        $(td).attr('colSpan', 7);
