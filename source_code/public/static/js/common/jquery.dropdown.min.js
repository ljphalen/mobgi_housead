;(function($) {
  ...
  var settings = {
    ...
    searchable: true,
    searchNoData:
      '<li style="color:#black">查无数据，换个词儿试试 /(ㄒoㄒ)/~~</li>',
    init: noop,
    choice: noop,
    extendProps: [],
    addSubmit: noop
  ...
  function maxItemAlert() {
    ...
    if ($alert.length === 0) {
      $alert = $(
        '<div class="dropdown-maxItem-alert">\u6700\u591A\u53EF\u9009\u62E9' +
          _config.limitCount +
          '\u4E2A</div>'
      )
    }
  ...
  function selectToDiv(str) {
    ...
    result = result.replace(/<option(.*?)<\/option>/gi, function(matcher) {
      ...
      var extendAttr = ''
      var extendProps = matcher.replace(/data-(\w+)="?(.[^"]+)"?/g, function(
        $1
      ) {
        extendAttr += $1 + ' '
      })
  ...
  var action = {
    add: function(event) {
      //console.log('adfhkjashfkjashfkjashfdkj')
      var _dropdown = this
      var _config = _dropdown.config
      var $el = _dropdown.$el
      var intputValue = $el.find('.labelSearch').val()

      // _config.addSubmit.call(_dropdown, event, intputValue)
      ...
      var text = $(event.target)
        .parent()
        .find('.childSelect option:selected')
        .text()
      if (!pid) {
        layer.msg('请选择一级标签!')
        return false
      ...
      $.ajax({
        url: 'addLevel2Label',
        async: false,
        data: { pid: pid, title: intputValue },
        type: 'get',
        dataType: 'json',
        jsonp: 'callback',
  ...
  Dropdown.prototype = {
    ...
    bindEvent: function() {
      ...
      $el.on(EVENT_SPACE.click, '.addlabel', $.proxy(action.add, _this))
    ...
    unbindEvent: function() {
      ...
      $el.off(EVENT_SPACE.click, '.addlabel')
